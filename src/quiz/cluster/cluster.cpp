...
void clusterHelper(const std::vector<std::vector<float>> points, std::vector<int>& cluster, std::vector<bool>& pointsCheck , int pointIdx, KdTree* tree, float distanceTol, unsigned char pointDim)
{	
	pointsCheck[pointIdx] = true;
	cluster.push_back(pointIdx);
	std::vector<int> nearByPointIdicies = tree->search(points[pointIdx], distanceTol, pointDim);
	
	for(int nearByIdx:nearByPointIdicies){
		if (pointsCheck[nearByIdx]==false)
			clusterHelper(points, cluster, pointsCheck, nearByIdx, tree, distanceTol, pointDim);
	}
}
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol, unsigned char pointDim)
{
	// TODO: Fill out this function to return list of indices for each cluster
	std::vector<std::vector<int>> clusters;
	std::vector<bool> pointsCheck(points.size(), false);
	for( int i =0; i<points.size(); i++ ){
		if (pointsCheck[i] == false){
			std::vector<int> cluster;
			clusterHelper(points, cluster, pointsCheck, i, tree, distanceTol, pointDim);
...
int main ()
{
	...
	// 2D points
	unsigned char pointDimension = 2;

    for (int i=0; i<points.size(); i++) 
    	tree->insert(points[i],i,pointDimension); 

  	int it = 0;
  	render2DTree(tree->root,viewer,window, it);
  
  	std::cout << "Test Search" << std::endl;
	unsigned char pointDim = 2;
  	std::vector<int> nearby = tree->search( {-6,7},3.0, pointDim);
  	...
  	//
  	std::vector<std::vector<int>> clusters = euclideanCluster(points, tree, 3.0, pointDim);
  	//
